...
var TONCrypto = require('mtproto-crypto').default;
...
KeyStore.prototype._generatePrivKeys = function(pwDerivedKey, n, handler) {
...
KeyStore.deriveKeyFromPasswordAndSalt = function(password, salt, callback) {
  ...
  var cb = function(derKey) {
    ...
    callback(error, ui8arr);
  }

  if (TONCrypto && TONCrypto.scrypt) {
    TONCrypto.scrypt(password, salt, logN, r, p, dkLen).then((hexDerKey) => {
        const derKey = Buffer.from(hexDerKey, 'hex').buffer;
        cb(derKey);
    }).catch((error) => {
        callback(error);
    });
...
KeyStore.prototype.generateNewAddress = function(pwDerivedKey, n, handler) {
  ...
  this._generatePrivKeys(pwDerivedKey, n, (keyObj, stopKeysGeneration) => {
